	...
	<div>
		...
		<div class="order">
			<div style="margin-bottom: 15px;font-size: 16px;color: rgba(240, 38, 69, 1);font-weight: 600;">
				{{ $t("my_order.order_list.order_generated") }}
			</div>
			<div class="ordercard">
				<div class="contract" @click="moveContact">
					...
					<span style="padding-left:2px" v-if="order.type == 'buy' && merchantid == order.merchant_id"
						@click="moveContact(order.user_id)">{{ $t("my_order.order_list.contact_buyer") }}</span>
					<span style="padding-left:2px" v-if="order.type == 'buy' && merchantid != order.merchant_id"
						@click="moveContact(order.merchant_id)">{{ $t("order_detail.contact") }}</span>
					<span style="padding-left:2px" v-if="order.type == 'sell' && merchantid == order.merchant_id"
						@click="moveContact(order.user_id)">{{ $t("my_order.order_list.contact_seller") }}</span>
					<span style="padding-left:2px" v-if="order.type == 'sell' && merchantid != order.merchant_id"
						@click="moveContact(order.merchant_id)">{{ $t("order_detail.contact") }}</span>
				...
				<div class="allcard">
					<span v-if="order.type == 'buy' && merchantid == order.merchant_id">{{ $t("order_detail.buyer") }}</span>
