...
export const urlToBase64OfList = async (list, key) => {
  ...
  if (type === 'String') {
    if (list.startsWith('http')) {
      const file = await getFileFromUrl(list)
      if (list && await isEnfile(file)) {
       const base64 = await decryption(file)
       return base64
      } else {
        return list
      }
  ...
  if (type === 'Object') {
    ...
    if (list[key].startsWith('http')) {
      const file = await getFileFromUrl(list[key])
      if (list[key] && await isEnfile(file)) {
        list[`base64${key}`] = await decryption(file)
      } else {
        list[`base64${key}`] = list[key]
      }
    } else {
      list[`base64${key}`] = list[key]
    }
